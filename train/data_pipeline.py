...
class _TokenizeFn:
    """Batched tokenization callable for .map()/.with_transform().

    A top-level class rather than a closure so it stays picklable - DataLoader
    workers pickle the dataset (and its transform) when num_workers > 0.
    """

    def __init__(self, tokenizer: PreTrainedTokenizer, text_column: str, max_seq_len: int):
...
def _create_tokenize_function(tokenizer: PreTrainedTokenizer, text_column: str, max_seq_len: int) -> Callable:
    """Creates a batched, picklable tokenization callable."""
    return _TokenizeFn(tokenizer, text_column, max_seq_len)
