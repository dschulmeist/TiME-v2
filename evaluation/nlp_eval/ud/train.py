    ...
    def __call__(self, sentences):
        ...
        return {
            ...
            "subwords": torch.stack(
                [F.pad(sentence["subwords"], (0, longest_source - sentence["subwords"].size(0)), value=self.pad_index)
                 for sentence in sentences]),
            "alignment": torch.stack(
                [
                    F.pad(F.one_hot(sentence["alignment"], num_classes=longest_target + 2).float(),
                          (0, 0, 0, longest_source - sentence["alignment"].size(0)), value=0.0)
                    for sentence in sentences
                ]
            ),
            "is_unseen": torch.stack(
                [F.pad(sentence["is_unseen"], (0, longest_target - sentence["is_unseen"].size(0)), value=False) for
                 sentence in sentences]),
            "lemma": {
                key: torch.stack(
                    [F.pad(sentence["lemma"][key], (0, longest_target - sentence["lemma"][key].size(0)), value=-1) for
                     sentence in sentences])
                for key in sentences[0]["lemma"].keys()
            },
            "upos": torch.stack(
                [F.pad(sentence["upos"], (0, longest_target - sentence["upos"].size(0)), value=-1) for sentence in
                 sentences]),
            "xpos": torch.stack(
                [F.pad(sentence["xpos"], (0, longest_target - sentence["xpos"].size(0)), value=-1) for sentence in
                 sentences]),
            "feats": torch.stack(
                [F.pad(sentence["feats"], (0, longest_target - sentence["feats"].size(0)), value=-1) for sentence in
                 sentences]),
            "arc_head": torch.stack(
                [F.pad(sentence["arc_head"], (0, longest_target - sentence["arc_head"].size(0)), value=-1) for sentence
                 in sentences]),
            "arc_dep": torch.stack(
                [F.pad(sentence["arc_dep"], (0, longest_target - sentence["arc_dep"].size(0)), value=-1) for sentence in
                 sentences]),
            ...
            "aux_feats_classes": {
                key: torch.stack([F.pad(sentence["aux_feats_classes"][key],
                                        (0, longest_target - sentence["aux_feats_classes"][key].size(0)), value=-1) for
                                  sentence in sentences])
...
def load_data(args, tokenizer):
    language_treebank_mapping = json.load(open(f"language_treebank_mapping_{args.version}.json", "r", encoding="utf-8"))
    ...
    train_data = Dataset(f"{treebank_path}/{train_filename}", partition='train', tokenizer=tokenizer, add_sep=True,
                         random_mask=True, min_count=args.min_count)
    dev_data = Dataset(f"{treebank_path}/{dev_filename}", partition='dev', tokenizer=tokenizer,
                       forms_vocab=train_data.forms_vocab, lemma_vocab=train_data.lemma_vocab,
                       upos_vocab=train_data.upos_vocab, xpos_vocab=train_data.xpos_vocab,
                       feats_vocab=train_data.feats_vocab, arc_dep_vocab=train_data.arc_dep_vocab, add_sep=True,
                       random_mask=False) if dev_filename is not None else None
    test_data = Dataset(f"{treebank_path}/{test_filename}", partition='test', tokenizer=tokenizer,
                        forms_vocab=train_data.forms_vocab, lemma_vocab=train_data.lemma_vocab,
                        upos_vocab=train_data.upos_vocab, xpos_vocab=train_data.xpos_vocab,
                        feats_vocab=train_data.feats_vocab, arc_dep_vocab=train_data.arc_dep_vocab, add_sep=True,
                        random_mask=False)
...
def main():
    ...
    parser.add_argument("--model", default="hf", choices=("hplt", "mbert", "xlmr", "hf"))
    ...
    parser.add_argument("--treebank_path", default="./ud_data/ud-treebanks-v2.15")
    parser.add_argument("--version", type=str, default="2_0",
                        choices=('2_0', '1_2'))  # treebank-mapping version
    parser.add_argument('--models_path', default='./',  # only used for --model hplt
                        help="base dir for HPLT model families (--model hplt only)")
    parser.add_argument("--results_path", default="./results/ud/")
    parser.add_argument("--checkpoints_path", default="./results/ud/checkpoints/")
    parser.add_argument("--custom_model_path", type=str, default=None,
                        help="Direct path to a pretrained model to be used as backbone.")
    ...
    if args.model == "hplt":
        ...
        args.model = args.custom_model_path.split("/")[-1]
    ...
    if args.log_wandb:
        wandb.init(name=f"{args.model.split('/')[-1]}_{args.language}", config=args, project="HPLT_UD", entity="ltg",
    ...
    train_loader = DataLoader(train_data, args.batch_size, shuffle=True, drop_last=True, num_workers=0,
                              collate_fn=CollateFunctor(train_data.pad_index))
    dev_loader = DataLoader(dev_data, args.batch_size, shuffle=False, drop_last=False, num_workers=0,
                            collate_fn=CollateFunctor(train_data.pad_index)) if dev_data is not None else None
    test_loader = DataLoader(test_data, args.batch_size, shuffle=False, drop_last=False, num_workers=0,
                             collate_fn=CollateFunctor(train_data.pad_index))
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_iter:
            ...
            aux_feats_loss = [
                (l * ((batch["aux_feats_classes"][key] != -1).float().sum().item() / (
                            batch["feats"] != -1).float().sum().item())) if (batch["aux_feats_classes"][
                                                                                 key] != -1).float().sum().item() > 0 else 0.0
        ...
        for loader_index, (loader, dataset) in enumerate([(dev_loader, dev_data), (test_loader, test_data)]):
            ...
            with torch.no_grad():
                ...
                dev_file = parse(open(dataset.path, "r", encoding="utf-8").read())
                    ...
                    for batch in loader:
                        ...
                        for i, index in enumerate(batch["index"]):
                            ...
                            for j, form in enumerate(dataset.forms[index]):
                                ...
                                lemma_rule = {
                                    rule_type: dataset.lemma_vocab[rule_type].get(p[i, j, :].argmax().item(),
            ...
            if loader_index == 0:
                ...
                print("UPOS: ", evaluation["UPOS"].aligned_accuracy * 100, "XPOS: ",
                      evaluation["XPOS"].aligned_accuracy * 100, flush=True)
                print("UFeats: ", evaluation["UFeats"].aligned_accuracy * 100, "AllTags: ",
                      evaluation["AllTags"].aligned_accuracy * 100, flush=True)
                print("Lemmas: ", evaluation["Lemmas"].aligned_accuracy * 100, "UAS: ",
                      evaluation["UAS"].aligned_accuracy * 100, flush=True)

                print("MLAS: ", evaluation["MLAS"].aligned_accuracy * 100, "BLEX: ",
                      evaluation["BLEX"].aligned_accuracy * 100, flush=True)
                mlas_blex_sum = evaluation["MLAS"].aligned_accuracy + evaluation["BLEX"].aligned_accuracy
                print(f"MLAS + BLEX: {mlas_blex_sum}", flush=True)
                if mlas_blex_sum < best_mlas_blex_sum:
                    print(f"Early stopping at epoch {epoch + 1}")
                    early_stopping = True
                    break
                else:
                    print(f"Best MLAS + BLEX: {best_mlas_blex_sum}", flush=True)
                    if mlas_blex_sum > best_mlas_blex_sum:
                ...
                with open(f"{args.results_path}{args.language}_{args.model}.jsonl", "w", encoding="utf-8") as f:
