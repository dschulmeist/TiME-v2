...
# optional sample caps for a fast pipeline check (None defaults can't go in
# default_params, whose argparse type is inferred from the value)
...
ds, lang = data_args.dataset_name.rsplit("/", 1)
if ds == "wikiann":  # the canonical repo moved to a namespaced id
    ds = "unimelb-nlp/wikiann"
if len(lang) > 2:
...
# optional sample caps for a fast pipeline check
if getattr(data_args, "max_train_samples", None) and "train" in dsd:
    dsd["train"] = dsd["train"].select(range(min(data_args.max_train_samples, len(dsd["train"]))))
if getattr(data_args, "max_eval_samples", None):
    for _split in ("validation", "test"):
        if _split in dsd:
            dsd[_split] = dsd[_split].select(range(min(data_args.max_eval_samples, len(dsd[_split]))))
...
if "LtgbertForMaskedLM" in config.architectures:
    print("Detected HPLT model. Applying manual build logic...")

    # 1. Load the BASE model. This works because the checkpoint has all the necessary
    #    weights for LtgbertModel, so no buggy initialization is triggered.
    # We need to import AutoModel specifically for this.
    from transformers import AutoModel

    print("Step 1: Loading the pretrained base LtgbertModel...")
    ...
    # 2. Create the full Token Classification model structure from the config.
    #    At this point, its weights are completely random.
    print("Step 2: Creating a randomly initialized LtgbertForTokenClassification model structure...")
    model = AutoModelForTokenClassification.from_config(
        config,
        trust_remote_code=True
    )

    # 3. Manually copy the pretrained weights from the base model into the
    #    full model structure, replacing the random base layers.
    #    The classification head remains randomly initialized, which is correct.
    print("Step 3: Copying pretrained base weights into the full model...")
    # The custom code defines the base layers under these attribute names
    model.embedding = base_model.embedding
    model.transformer = base_model.transformer

    # 4. Clean up the base model to save memory.
    del base_model
    import gc

    gc.collect()
    torch.cuda.empty_cache()
    print("Successfully built LtgbertForTokenClassification model.")

# This logic handles all other "normal" models. It remains unchanged.
else:
    print("Detected standard model. Using default from_pretrained...")
...
# transformers >=5 validates label2id as dict[str, int]
model.config.label2id = {str(l): i for i, l in enumerate(label_list)}
model.config.id2label = {i: str(l) for i, l in enumerate(label_list)}
...
print(f"Model: {modelname}, Dataset: {datasetname}, Score: {score:.3f}")
