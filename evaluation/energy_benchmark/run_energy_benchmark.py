...
Models are given explicitly with --models and may be any mix of HF hub ids,
local checkpoint directories, and per-language templates (a "{lang}" placeholder
is expanded against --langs, for per-language model families) - the same
checkpoints the distillation recipes produce, so energy lines up with quality.

Example:
  python run_energy_benchmark.py \
    --models FacebookAI/xlm-roberta-base ./models/german_moderngbert/student/checkpoint-6000 \
    --langs de en \
    --output results.csv --fp16 --enable-power

  # a {lang} template is expanded per language (per-language model families):
  python run_energy_benchmark.py --models "your-org/encoder-{lang}" --langs de en fr
...
def _logical_to_nvml_ids(logical_ids: List[int]) -> List[int]:
    ...
    for li in logical_ids:
        try:
            ...
            logging.warning("Could not map logical GPU %d via CUDA_VISIBLE_DEVICES=%r", li, cvd)
            out.append(li)
    return out


def _trapezoid_energy_J(samples: List[tuple]) -> float:
    """Trapezoidal integration of (timestamp_s, power_mW) samples to Joules."""
    if len(samples) < 2:
        return 0.0
    E = 0.0
...
class _PowerPollerNVML:
    ...
    def energy_J(self) -> float:
        return _trapezoid_energy_J(self.samples)
    ...
    def __enter__(self):
        ...
        if NVML_OK:
            try:
                ...
                logging.debug("NVML total-energy counter unavailable: %s", e)
            try:
                ...
                logging.debug("NVML power polling unavailable: %s", e)
        try:
            self._poller = _PowerPollerSMI(logical_gpu_index=0, interval_s=self.poll_interval_s)
            self._poller.start()
            self.mode = "smi_poll"
        except Exception as e:
            logging.warning("nvidia-smi power polling unavailable, energy metering disabled: %s", e)
            self.mode = "disabled"
        return self
        ...
        try:
            ...
            if self._nvml_inited:
                try:
                    ...
                    logging.debug("nvmlShutdown failed: %s", e)
...
DEFAULT_CACHE_DIR = Path("sentence_cache_multilingual_wikipedia")

_SEGMENTER_CACHE: dict[str, "pysbd.Segmenter"] = {}

def _get_segmenter(lang_code: str) -> "pysbd.Segmenter":
    seg = _SEGMENTER_CACHE.get(lang_code)
    if seg is None:
        seg = pysbd.Segmenter(language=lang_code, clean=False)
        _SEGMENTER_CACHE[lang_code] = seg
    return seg

def sent_seg(text: str, lang_code: str) -> list[str]:
    """Segment text into sentences; fall back to a regex splitter."""
    if lang_code == "ur":
        lang_code = "hi"  # heuristic: use hi segmenter for Urdu
    if lang_code in PYSBD_SUPPORTED:
        try:
            sents = _get_segmenter(lang_code).segment(text)
        except Exception:
...
def stream_sentences(d_name: str, d_config: str, lang_code: str, n_target: int,
                     cache_dir: Path = DEFAULT_CACHE_DIR) -> list[str]:
    """Stream sentences from a HF dataset (Wikipedia) and cache them to disk."""
...
def benchmark_one(
    ...
    for bs in batch_sizes:
        ...
        # Invariant across repeats; computed outside any timed/metered region.
        ...
        for _ in range(repeats):
            ...
            energies.append(E)
            powers.append(P)
            jps.append(float("nan") if math.isnan(E) else E / samples_measured)
            jpt.append(float("nan") if math.isnan(E) else E / tokens_measured_total)
...
def parse_args():
    ...
    p.add_argument("--models", nargs="+", required=True,
                   help="Models to benchmark: HF hub ids, local checkpoint dirs, or "
                        "per-language templates with a {lang} placeholder (expanded "
                        "against --langs). Mix freely, e.g. "
                        "'FacebookAI/xlm-roberta-base ./models/foo/student/checkpoint-6000'.")
    ...
    p.add_argument("--cache-dir", type=Path, default=DEFAULT_CACHE_DIR,
                   help="Directory for cached sentence pools")
    return p.parse_args()
...
def main():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                logging.debug("set_float32_matmul_precision unavailable: %s", e)
        torch.set_num_threads(1)
    except Exception as e:
        logging.warning("Could not configure torch backend knobs: %s", e)
    ...
    # Models: HF ids / local dirs / {lang} templates, expanded per language below.
    model_templates = args.models

    # Seed for reproducible sentence sampling
    random.seed(1337)
    try:
        import numpy as np
        np.random.seed(1337)
