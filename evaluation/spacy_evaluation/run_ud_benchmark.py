#!/usr/bin/env python3
"""Train and evaluate spaCy pipelines on Universal Dependencies treebanks.

Downloads UD data, trains custom HF-encoder pipelines and spaCy baselines,
and evaluates them with `spacy benchmark accuracy` on CPU (and GPU if requested).
"""
import argparse, json, os, pathlib, re, shutil, subprocess, sys, zipfile, urllib.request, importlib
...
def _curated_arch_for_name(name: str):
    """Pick the spacy-curated-transformers architecture for an encoder name."""
    low = name.lower()
    if "roberta" in low:
...
_VALIDATE_CACHE = {"out": None}

def has_model(mname, refresh=False):
    # `spacy validate` spawns a subprocess and fetches the compatibility table;
    # cache its output so we don't pay that cost once per baseline model.
    if refresh or _VALIDATE_CACHE["out"] is None:
        try:
            _VALIDATE_CACHE["out"] = subprocess.check_output(
                [sys.executable, "-m", "spacy", "validate"]).decode()
        except subprocess.CalledProcessError:
            _VALIDATE_CACHE["out"] = ""
    return mname in _VALIDATE_CACHE["out"]
...
def convert_ud(lang, ud_dir):
    for split in ("train","dev","test"):
        ...
        if dst.exists() and src.exists() and dst.stat().st_mtime >= src.stat().st_mtime:
            print(f"[ok] {dst} up-to-date, skip convert")
            continue
...
# ===== initializers =====
def _spacy_init_config(lang, cfg_path):
    run([
...
def _print_full_config(cfg_path, label):
    print("\n" + "="*80)
    print(f"FULL TRAINING CONFIG ({label}): {cfg_path}")
    print("-"*80)
    print(pathlib.Path(cfg_path).read_text())
    print("="*80 + "\n")

def _init_config_transformer(lang, encoder_name, cfg_path, curated):
    """Build a transformer-based training config (curated or spacy-transformers)."""
    _spacy_init_config(lang, cfg_path)
    text = pathlib.Path(cfg_path).read_text()
    if curated:
        text = _ensure_trf_component_curated(text, encoder_name)
    else:
        text = _ensure_trf_component_spacy_transformers(text, encoder_name)
    text = _ensure_pipeline(text, ["transformer","morphologizer","tagger","parser","senter"])
    text = _remove_tok2vec_everywhere(text)
    force_listener = _force_listener_curated if curated else _force_listener_spacy_transformers
    for comp in ("morphologizer","tagger","parser","senter"):
        text = force_listener(text, f"components.{comp}")
    ...
    _print_full_config(cfg_path, "curated" if curated else "spacy-transformers")

def init_config_transformer_spacy_tf(lang, transformer_name, cfg_path):
    _init_config_transformer(lang, transformer_name, cfg_path, curated=False)

def init_config_transformer_curated(lang, encoder_name, cfg_path):
    _init_config_transformer(lang, encoder_name, cfg_path, curated=True)
...
def _maybe_tok2vec_path(package_name):
    try:
        ...
        print(f"Note: could not locate tok2vec weights in package '{package_name}': {e}")
        return None
...
def init_config_tok2vec(lang, cfg_path, init_from_package=None):
    ...
    pathlib.Path(cfg_path).write_text(text)
    _print_full_config(cfg_path, "tok2vec")
...
def evaluate_cpu_gpu(model_path_or_name, test_spacy, out_json_base, gpu_id=None):
    ...
    run(cmd)
    print(f"[ok] CPU metrics saved to {out_cpu}")
    ...
    if gpu_id is not None:
        ...
        cmd = [sys.executable, "-m", "spacy", "benchmark", "accuracy",
               ...
               "--gpu-id", str(gpu_id)]
        run(cmd)
        print(f"[ok] GPU metrics saved to {out_gpu}")

# ===== main =====
def main():
    ap = argparse.ArgumentParser(description="Train and evaluate spaCy pipelines on UD treebanks.")
    ap.add_argument("--langs", default="en,de,fr,es,it,da", help="Comma-separated language codes")
    ap.add_argument("--workdir", default=".", help="Working directory for assets/, models/, metrics/, configs/")
    ap.add_argument("--hf-json", default="hf_models.json", help="JSON map of HF encoders per language")
    ap.add_argument("--cpu", action="store_true", help="Force CPU (ignore --gpu)")
    ap.add_argument("--gpu", type=int, default=None, help="GPU id for training/evaluation")
    ap.add_argument("--bench-only", action="store_true",
                    help="Only run benchmarks (no training). Expects existing models in models/*.")
        ...
        for lang in langs:
            ...
            for sub in sorted(models_dir.glob(f"{lang}_*")):
                ...
                if args.resume and metrics_exist(metrics_base, require_gpu=(gpu_id is not None)):
                    print(f"[skip]  Resume: metrics already exist for {sub.name}, skipping benchmark")
                    continue
    ...
    # Download spaCy baselines if missing
    for lang in langs:
    ...
    for lang in langs:
        ...
        if args.resume and all_models_trained_for_lang(lang, encoders, baselines, models_dir):
            print(f"[skip]  Resume: all models for '{lang}' already trained. Skipping language.")
            continue

        # Train HF encoders (transformer pipelines)
        if encoders:
            for enc in encoders:
                name = enc["id"]
                ...
                if args.resume and model_best_exists(out_dir):
                    print(f"[skip]  Resume: {out_dir.name} already has model-best/, skipping training")
                else:
                ...
                metrics_base = metrics_dir/f"ud_{lang}_custom_{slug}.json"
                if args.resume and metrics_exist(metrics_base, require_gpu=(gpu_id is not None)):
                    print(f"[skip]  Resume: metrics exist for {out_dir.name}, skipping evaluation")
                else:
        ...
        # Train spaCy baselines (finetune)
        for m in baselines:
            slug = slugify(m)
            out_dir = models_dir/f"{lang}_spacy_{slug}"
                ...
                if args.resume and model_best_exists(out_dir):
                    ...
                    if USE_CURATED_FOR_TRF_BASELINES:
                        try:
                            ...
                            print(f"[warn] Curated setup failed for '{m}' ({e}). Falling back to spacy-transformers.")
