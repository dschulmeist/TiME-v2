...
DEFAULT_SENT_CACHE = Path("sentence_cache_wikipedia_spacy")
DEFAULT_OUTPUT_CSV = Path("metrics/latency_throughput_wikipedia.csv")
...
def _get_segmenter(lang: str):
    seg = _SEGMENTER_CACHE.get(lang)
    if seg is None:
        seg = pysbd.Segmenter(language=lang, clean=False)
        _SEGMENTER_CACHE[lang] = seg
    return seg

def sentences_from_text(text: str, lang: str) -> list[str]:
    """Segment text into sentences with pysbd; fall back to a regex splitter."""
    sents = None
    if pysbd is not None:
        lang_for_pysbd = "hi" if lang == "ur" else lang  # heuristic: hi segmenter for Urdu
        if lang_for_pysbd in PYSBD_SUPPORTED_LANGS:
            try:
                sents = _get_segmenter(lang_for_pysbd).segment(text)
            except Exception as e:
                logging.debug("pysbd segmentation failed for lang=%s, using regex fallback: %s", lang, e)
    if sents is None:
        sents = re.split(r'(?<=[.!?۔])\s+', text)
    return [s.strip() for s in sents if len(s.split()) >= 3]

def stream_sentences(lang: str, need: int, cache_dir: Path = DEFAULT_SENT_CACHE) -> list[str]:
    if lang not in WIKI:
    ...
    cache_file = cache_dir / f"{lang}_{d_name.replace('/','_')}_{d_cfg}_{need}.txt"
...
def _append_csv(rows: Iterable[Row], csv_path: Path) -> None:
...
def _bench_models_on_device(model_sources: Sequence[Tuple[str, str]], lang: str, device: str,
                            pool: list[str], args, done_set: Set[Tuple[str, str, str, int]],
                            output_csv: Path) -> None:
    """Benchmark all matching model sources for one language on one device."""
    for label, src in model_sources:
        ...
        if declared_lang is not None and declared_lang != lang.lower():
            ...
            continue
        if args.resume and done_set and all(
            (src, lang, device, bs) in done_set for bs in args.batch_sizes
        ):
            logging.info("-> SKIP %s for lang=%s [%s]: all batch sizes already in CSV (resume)",
                         label, lang, device)
            continue
        nlp = _load_nlp(src)
        if nlp is None:
        ...
        if declared_lang is None and getattr(nlp, "lang", None) and nlp.lang.lower() != lang.lower():
            ...
            continue
        logging.info(f"\n-> {device.upper()}: {label}")
        rows = _bench_one(nlp, label, src, lang, device, pool, args.batch_sizes, args.repeats,
...
def main():
    ...
    ap.add_argument("--langs", nargs="*", default=DEFAULT_LANGS, help="Languages to benchmark (Wikipedia codes)")
    ap.add_argument("--batch-sizes", nargs="*", type=int, default=DEFAULT_BATCH_SIZES, help="Batch sizes to sweep")
    ap.add_argument("--repeats", type=int, default=7, help="Repeats per (model, batch size)")
    ap.add_argument("--device", choices=["cpu","gpu","both"], default="both", help="Device(s) to benchmark on")
    ap.add_argument("--gpu", type=int, default=None, help="GPU index (sets CUDA_VISIBLE_DEVICES)")
    ap.add_argument("--models-glob", default="models/*/model-best", help="Glob for spaCy pipeline directories")
    ap.add_argument("--no-models-glob", action="store_true", help="Do not expand --models-glob")
    ap.add_argument("--packages", nargs="*", default=[], help="Installed spaCy package names to benchmark")
    ap.add_argument("--resume", action="store_true", help="Skip (model, lang, device, batch size) rows already in the output CSV")
    ap.add_argument("--threads", type=int, default=None, help="Limit BLAS/OpenMP thread count")
    ap.add_argument("--output", type=Path, default=DEFAULT_OUTPUT_CSV, help="Output CSV path")
    ap.add_argument("--cache-dir", type=Path, default=DEFAULT_SENT_CACHE, help="Directory for cached sentence pools")
    args = ap.parse_args()
    ...
    done_set: Set[Tuple[str,str,str,int]] = _load_done(args.output) if args.resume else set()

    for lang in args.langs:
        need = max(args.batch_sizes) * TOTAL_BATCHES
        try:
            pool = stream_sentences(lang, need, cache_dir=args.cache_dir)
        except Exception as e:
        ...
        if args.device in ("cpu","both") or not have_gpu:
            _bench_models_on_device(model_sources, lang, "cpu", pool, args, done_set, args.output)

        if have_gpu and args.device in ("gpu","both"):
            _bench_models_on_device(model_sources, lang, "gpu", pool, args, done_set, args.output)

    logging.info(f"\nAll done. Results appended to {args.output}")
